...
class ProviderPool:
    ...
    def get_stats(self) -> dict:
        return {
            ...
            "active": len(
                [c for c in self.credentials if c.status == CredentialStatus.ACTIVE]
            ),
            "cooldown": len(
                [c for c in self.credentials if c.status == CredentialStatus.COOLDOWN]
            ),
            "exhausted": len(
                [c for c in self.credentials if c.status == CredentialStatus.EXHAUSTED]
            ),
            "invalid": len(
                [c for c in self.credentials if c.status == CredentialStatus.INVALID]
            ),
...
class CredentialPool:
        ...
        if self.pool_file.exists():
            try:
                ...
                for pdata in data.get("providers", []):
                    creds = [
                        Credential.from_dict(c) for c in pdata.get("credentials", [])
                    ]
                    pool = ProviderPool(
    ...
    def _save(self):
        tmp_fd, tmp_path = tempfile.mkstemp(
            dir=str(self.pool_file.parent), suffix=".tmp"
        )
        try:
            data = {
                "providers": [
    ...
    def record_success(
        self, cred_id: str, provider: str, tokens: int = 0, cost: float = 0.0
    ):
        with self._lock:
            if provider in self.providers:
                for c in self.providers[provider].credentials:
                    if c.cred_id == cred_id:
                        c.record_success(tokens, cost)
                        break
                self._save()

    def record_error(
        self, cred_id: str, provider: str, error_code: int = 0, error_msg: str = ""
    ):
        with self._lock:
            if provider in self.providers:
                for c in self.providers[provider].credentials:
                    if c.cred_id == cred_id:
                        c.record_error(error_code, error_msg)
                        break
                self._save()
    ...
    def health_check(self) -> dict:
        ...
        report = {"providers": {}, "total_active": 0, "total_keys": 0, "warnings": []}
        for name, pool in self.providers.items():
            ...
            if stats["cooldown"] > 0:
                report["warnings"].append(
                    f"{name}: {stats['cooldown']} keys in cooldown"
                )
        return report
    ...
    def get_stats(self) -> dict:
        return {
            ...
            "total_active": sum(
                len([c for c in p.credentials if c.is_available()])
                for p in self.providers.values()
            ),
        }
