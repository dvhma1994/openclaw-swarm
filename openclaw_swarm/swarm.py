...
class SwarmCoordinator:
    ...
    def __init__(
        ...
        self.storage_path = Path(
            storage_path
            or os.path.join(os.path.dirname(__file__), "..", "..", "data", "swarm")
        )
    ...
    def register_agent(self, agent: SwarmAgent) -> None:
        ...
        self.agents[agent.id] = agent
        console.print(
            f"[green]Agent registered: {agent.name} ({agent.role.value})[/green]"
        )

    def decompose_task(
        self, task_description: str, num_subtasks: int = 3
    ) -> List[SwarmTask]:
        """
...
"""
        ...
        if not subtasks:
            subtasks.append(
                self._create_task(
                    {"description": task_description, "priority": TaskPriority.MEDIUM}
                )
            )
    ...
    def _create_task(self, task_data: Dict[str, Any]) -> SwarmTask:
        ...
        task_id = hashlib.md5(
            f"{task_data['description']}:{datetime.now().isoformat()}".encode()
        ).hexdigest()[:12]
    ...
    def assign_agent(self, task: SwarmTask) -> Optional[str]:
        ...
        advice = self.experience.get_advice(
            task.description.split()[0] if task.description else "general"
        )

        # Prefer agents with good experience
        for agent in available:
            if any(
                cap in " ".join(advice.get("best_practices", []))
    ...
    def execute_task(
        self, task: SwarmTask, agent_id: str, context: Optional[str] = None
    ) -> str:
        ...
        memory_context = "\n".join(
            [f"Past experience: {m.output_data[:200]}" for m in memories]
        )
...
Please complete this task."""
        ...
        if success:
            agent.success_rate = (
                agent.success_rate * (agent.total_completed - 1) + 1
            ) / agent.total_completed
        else:
            agent.success_rate = (
                agent.success_rate * (agent.total_completed - 1) / agent.total_completed
            )
    ...
    def run_swarm(
        self, task_description: str, max_workers: int = 3, decompose: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        """
        console.print(
            f"[bold blue]Starting swarm for: {task_description[:50]}...[/bold blue]"
        )
        ...
        with Progress(console=console) as progress:
            task_progress = progress.add_task(
                "[cyan]Executing tasks...", total=len(subtasks)
            )
            ...
            while len(completed) < len(subtasks):
                ...
                for task in ready[:max_workers]:
                    ...
                    if not agent_id:
                        console.print(
                            f"[red]No available agent for task {task.id}[/red]"
                        )
                        continue
        ...
        console.print(
            f"[green]Swarm completed {len(completed)}/{len(subtasks)} tasks[/green]"
        )
