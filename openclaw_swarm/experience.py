...
class ExperienceDB:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = Path(storage_path or os.path.join(
            os.path.dirname(__file__), "..", "..", "data", "experience"
        ))
    ...
    def _load(self) -> None:
        ...
        if self.experiences_file.exists():
            try:
                ...
                console.print(f"[yellow]Warning: Could not load experiences: {e}[/yellow]")
    ...
    def record_experience(
        ...
        console.print(f"[green]Experience recorded: {exp_id} ({'success' if success else 'failure'})[/green]")
        return exp_id
        ...
        if experience.success:
            ...
            if lesson_id in self.lessons:
                ...
                self.lessons[lesson_id].confidence = min(
                    1.0, 
                    self.lessons[lesson_id].confidence + 0.1
                )
            else:
                # Create new lesson
            ...
            lesson_text = f"When doing {experience.task_type}, avoid {experience.action_taken}"
            lesson_id = self._generate_id(lesson_text)
    ...
    def get_best_practices(self, task_type: str) -> List[str]:
        ...
        practices = [
            lesson.rule for lesson in lessons
        ...
        for lesson in lessons:
            if lesson.confidence >= 0.8:
                ...
                if proposed_action.lower() in lesson.rule.lower():
                    if "avoid" in lesson.rule.lower():
                        return (False, f"Past experience suggests avoiding: {lesson.rule}")
                    else:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_confidence": sum(e.confidence for e in self.experiences.values()) / total if total > 0 else 0
        }
