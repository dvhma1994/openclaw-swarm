...
class Orchestrator:
    ...
    def run_agent(
        ...
        try:
            ...
            # Call router
            task_type = TaskType(agent.model_type) if agent.model_type in [t.value for t in TaskType] else TaskType.GENERAL
    ...
    def run_workflow(
        ...
        if workflow is None:
            workflow = self.config.get("workflow", {}).get("default", ["planner", "coder", "reviewer"])
        ...
        if show_progress:
            with Progress(console=console) as progress:
                task = progress.add_task("[cyan]Running workflow...", total=len(workflow))
...
def orchestrate(prompt: str, workflow: Optional[List[str]] = None) -> Dict[str, AgentResult]:
    """Quick orchestration"""
