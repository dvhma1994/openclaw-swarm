...
class Anonymizer:
    ...
    def __init__(self, config_path: Optional[str] = None):
        self.config_path = Path(config_path or os.path.join(
        ...
        self.vault_path = Path(os.path.join(
            os.path.dirname(__file__), "..", "..", "data", "vault"
        ))
    ...
    def _save_to_vault(self, mapping: Dict[str, str]) -> None:
        ...
        vault_file = self.vault_path / f"session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        with open(vault_file, 'w', encoding='utf-8') as f:
            json.dump({
                ...
                "types": list(set(t.split("_")[0].replace("<PII_", "") for t in mapping.keys()))
            }, f, indent=2)
    ...
    def process_prompt(
        ...
        stats = {e.type: len([x for x in result.entities if x.type == e.type]) 
