...
class WorkingMemory:
    ...
    def _save(self) -> None:
        ...
        with open(file, 'w', encoding='utf-8') as f:
            json.dump({k: asdict(v) for k, v in self.items.items()}, f, indent=2, ensure_ascii=False)
...
class EpisodicMemory:
    ...
    def _generate_id(self, event: str) -> str:
        return hashlib.md5(f"{event}:{datetime.now().isoformat()}".encode()).hexdigest()[:8]
    ...
    def get_important(self, min_importance: float = 0.7, limit: int = 10) -> List[EpisodicMemoryItem]:
    ...
    def compress_to_semantic(self) -> List[Dict[str, Any]]:
        ...
        for tag, items in tag_groups.items():
            ...
            knowledge = {
                ...
                "confidence": min(1.0, total / 10),  # More experiences = higher confidence
            }
...
class SemanticMemory:
    ...
    def get_high_confidence(self, min_confidence: float = 0.8, limit: int = 10) -> List[SemanticMemoryItem]:
...
class MultiTierMemory:
    ...
    def __init__(self, base_path: Optional[str] = None):
        self.base_path = Path(base_path or os.path.join(
    ...
    def store_event(
        ...
        emotional = 1.0 if "success" in outcome.lower() else (-1.0 if "fail" in outcome.lower() else 0.0)
    ...
    def recall(self, query: str, limit: int = 5) -> Dict[str, List[Any]]:
        ...
        return {
            ...
            "episodic": [
                {"event": item.event, "outcome": item.outcome, "importance": item.importance}
                for item in self.episodic.search(query, limit)
            ],
            "semantic": [
                {"concept": item.concept, "knowledge": item.knowledge, "confidence": item.confidence}
