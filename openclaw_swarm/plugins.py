...
class PluginManager:
    ...
    def __init__(self, plugin_dir: Optional[str] = None):
        self.plugin_dir = Path(plugin_dir or os.path.join(
        ...
        try:
            ...
            if main_file.exists():
                ...
                if spec and spec.loader:
                    ...
                    console.print(f"[green]Loaded plugin: {metadata.name} v{metadata.version}[/green]")
                    return True
            else:
                ...
                self.plugins[plugin_id] = plugin
                console.print(f"[green]Loaded plugin (metadata only): {metadata.name}[/green]")
                return True
        ...
        for plugin in self.plugins.values():
            if plugin.metadata.enabled and hook_name in plugin.hooks:
                try:
                    ...
                    console.print(f"[yellow]Hook {hook_name} failed in {plugin.metadata.id}: {e}[/yellow]")
...
'''
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "enabled_plugins": sum(1 for p in self.plugins.values() if p.metadata.enabled),
            "disabled_plugins": sum(1 for p in self.plugins.values() if not p.metadata.enabled),
            "plugin_types": {
                t.value: sum(1 for p in self.plugins.values() if p.metadata.plugin_type == t.value)
                for t in PluginType
            },
            "hooks_registered": {
