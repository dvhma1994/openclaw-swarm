...
class SkillCategory(str, Enum):
    ...
    TESTING = "testing"; SECURITY = "security"
    INFRASTRUCTURE = "infrastructure"; CONTENT = "content"
    TRADING = "trading"; MONITORING = "monitoring"
    MEMORY = "memory"; DEBUGGING = "debugging"
    DEVOPS = "devops"; ANALYSIS = "analysis"
...
class SkillEntry:
    ...
    skill_id: str; name: str; description: str
    ...
    source_url: str = ""
    installs: int = 0; rating: float = 0.0
    compatible_tools: list = field(default_factory=list)  # ["claude", "cursor", "codex"]
    created_at: str = ""; updated_at: str = ""

    def to_dict(self):
        return {**asdict(self), "category": self.category.value, "risk": self.risk.value}
...
class SkillsMarketplace:
    ...
    def _save(self):
        tmp_fd, tmp_path = tempfile.mkstemp(dir=str(self.registry_file.parent), suffix='.tmp')
        try:
    ...
    def _register_builtin_skills(self):
        ...
        builtins = [
            SkillEntry(skill_id="brainstorming", name="Brainstorming",
                description="Plan features, MVPs, and product ideas using structured ideation",
                category=SkillCategory.DEVELOPMENT, risk=SkillRisk.SAFE,
            ...
            SkillEntry(skill_id="architect", name="Architecture Review",
                ...
                compatible_tools=["claude", "codex"]),
            SkillEntry(skill_id="test-writer", name="Test Writer",
                description="Automatically write unit tests for any code module",
                category=SkillCategory.TESTING, risk=SkillRisk.LOW,
                ...
                compatible_tools=["claude", "cursor", "codex"]),
            SkillEntry(skill_id="deployer", name="Deployer",
                description="Safe deployment with rollback capability and health checks",
                category=SkillCategory.DEVOPS, risk=SkillRisk.HIGH,
            ...
            SkillEntry(skill_id="perf-profiler", name="Performance Profiler",
                ...
                compatible_tools=["claude"]),
            SkillEntry(skill_id="memory-curator", name="Memory Curator",
                description="Manage and consolidate the dual-layer memory system",
                category=SkillCategory.MEMORY, risk=SkillRisk.SAFE,
    ...
    def publish(self, name: str, description: str, content: str,
                category: str = "development", risk: str = "safe",
                tags: list = None, author: str = "",
                compatible_tools: list = None) -> SkillEntry:
        ...
        skill = SkillEntry(
            ...
            category=SkillCategory(category), risk=SkillRisk(risk),
            content=content, tags=tags or [], author=author,
            compatible_tools=compatible_tools or ["claude"],
            created_at=datetime.now(timezone.utc).isoformat(),
            updated_at=datetime.now(timezone.utc).isoformat())
        self.skills[skill_id] = skill; self._save()
        return skill
    ...
    def install(self, skill_id: str, target_dir: Path = None) -> dict:
        ...
        skill.installs += 1; self._save()
        return {"success": True, "installed_to": str(skill_file),
    ...
    def install_bundle(self, bundle_category: str, target_dir: Path = None) -> dict:
        ...
        for skill in skills:
            ...
            if result["success"]: results["installed"] += 1
    ...
    def search(self, query: str = "", category: str = "",
               risk: str = "", tag: str = "",
               compatible_tool: str = "") -> List[SkillEntry]:
        ...
        if query:
            ...
            for s in results:
                score = 0
                if ql in s.name.lower(): score += 3
                if ql in s.description.lower(): score += 2
                if any(ql in t.lower() for t in s.tags): score += 1
    ...
    def get_stats(self) -> dict:
        return {
            ...
            "by_risk": {r: sum(1 for s in self.skills.values() if s.risk.value == r)
...
def get_marketplace() -> SkillsMarketplace:
    global _marketplace
    if _marketplace is None: _marketplace = SkillsMarketplace()
    return _marketplace
...
if __name__ == "__main__":
    ...
    if cmd == "stats": print(json.dumps(mp.get_stats(), indent=2))
    elif cmd == "search":
        results = mp.search(query=sys.argv[2] if len(sys.argv) > 2 else "",
                           category=sys.argv[3] if len(sys.argv) > 3 else "")
        for s in results:
            print(f"  [{s.category.value:15s}] {s.name}: {s.description[:60]}")
    elif cmd == "categories": print(json.dumps(mp.list_categories(), indent=2))
        ...
        print("Commands: stats, search [query] [category], categories, install <id>, install-bundle <category>")