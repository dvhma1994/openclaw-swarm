...
"""
import json, os, hashlib, math, time, tempfile, threading
...
class AutoMemoryEntry:
    entry_id: str; text: str; source: str = "conversation"
    sender: str = ""; timestamp: str = ""; session_id: str = ""
    tags: list = field(default_factory=list); embedding_hash: str = ""
    def to_dict(self): return asdict(self)

@dataclass
class ConsciousMemoryEntry:
    entry_id: str; content: str; tags: list = field(default_factory=list)
    importance: int = 5; source: str = "explicit"
    access_count: int = 0; last_accessed: str = ""
    created_at: str = ""; metadata: dict = field(default_factory=dict)
    def to_dict(self): return asdict(self)
    def touch(self):
        self.access_count += 1
...
@dataclass
class KnowledgeNode:
    node_id: str; entity: str; entity_type: str = "concept"
    properties: dict = field(default_factory=dict); created_at: str = ""
    def to_dict(self): return asdict(self)

@dataclass
class KnowledgeEdge:
    edge_id: str; source_id: str; target_id: str; relation: str = "related_to"
    weight: float = 1.0; evidence: str = ""; created_at: str = ""
    def to_dict(self): return asdict(self)
...
class AutomaticMemory:
    ...
    def __init__(self, store_path: Path = AUTO_FILE):
        ...
        self._lock = threading.Lock(); self._load()
        ...
        if self.store_path.exists():
            try:
                ...
                for e in data.get("entries", []):
                    entry = AutoMemoryEntry(**e); self.entries[entry.entry_id] = entry
            except Exception: pass

    def _save(self):
        tmp_fd, tmp_path = tempfile.mkstemp(dir=str(self.store_path.parent), suffix='.tmp')
        try:
            with os.fdopen(tmp_fd, 'w', encoding='utf-8') as f:
                json.dump({"entries": [e.to_dict() for e in self.entries.values()],
    ...
    def store(self, text: str, source: str = "conversation",
             sender: str = "", session_id: str = "", tags: list = None) -> str:
        eid = f"auto_{hashlib.md5(text[:200].encode()).hexdigest()[:8]}_{int(time.time())}"
        entry = AutoMemoryEntry(entry_id=eid, text=text, source=source,
            sender=sender, session_id=session_id, tags=tags or [],
            timestamp=datetime.now(timezone.utc).isoformat(),
            embedding_hash=hashlib.sha256(text.encode()).hexdigest()[:16])
        with self._lock: self.entries[eid] = entry; self._save()
        return eid

    def search(self, query: str, top_k: int = 5) -> List[dict]:
        query_terms = set(query.lower().split()); scored = []
    ...
    def consolidate_candidates(self, min_age_hours: int = 24) -> List[AutoMemoryEntry]:
        ...
        seen = {}
        for entry in self.entries.values():
            ts = datetime.fromisoformat(entry.timestamp).timestamp() if entry.timestamp else 0
            if ts < cutoff:
                seen.setdefault(entry.embedding_hash, []).append(entry)
        candidates = []
        for h, entries in seen.items():
            if len(entries) >= 2: candidates.extend(entries)
        return candidates
...
class ConsciousMemory:
    ...
    def save(self, content: str, tags: list = None,
        ...
        entry = ConsciousMemoryEntry(entry_id=eid, content=content,
            tags=tags or [], importance=max(1, min(10, importance)),
    ...
    def search(self, query: str = "", tags: list = None,
               min_importance: int = 1, top_k: int = 10) -> List[dict]:
        results = []
        query_terms = set(query.lower().split()) if query else set()
        for entry in self.entries.values():
            if entry.importance < min_importance: continue
            if tags and not any(t in entry.tags for t in tags): continue
            if query_terms:
                entry_terms = set(entry.content.lower().split())
                overlap = len(query_terms & entry_terms)
                if overlap == 0 and not tags: continue
    ...
    def consolidate_from_automatic(self, auto_mem: AutomaticMemory) -> int:
        ...
        for entry in candidates:
            ...
            if existing and existing[0]["score"] > 0.5: continue
            self.save(content=entry.text[:2000],
                      tags=entry.tags + [entry.source, "auto_consolidated"],
                      importance=6, source="consolidated",
    ...
    def get_stats(self):
        return {"total_entries": len(self.entries),
                "avg_importance": round(sum(e.importance for e in self.entries.values()) / max(len(self.entries), 1), 1),
                "by_source": {s: sum(1 for e in self.entries.values() if e.source == s)
...
class KnowledgeGraph:
    ...
    def _load(self):
        if self.store_path.exists():
            try:
                with open(self.store_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                for n in data.get("nodes", []): self.nodes[n["node_id"]] = KnowledgeNode(**n)
                for e in data.get("edges", []): self.edges[e["edge_id"]] = KnowledgeEdge(**e)
            except Exception: pass
    ...
    def _save(self):
        ...
        try:
            with os.fdopen(tmp_fd, 'w', encoding='utf-8') as f:
                json.dump({"nodes": [n.to_dict() for n in self.nodes.values()],
    ...
    def add_node(self, entity: str, entity_type: str = "concept",
                 properties: dict = None) -> str:
        nid = f"n_{hashlib.md5(entity.encode()).hexdigest()[:8]}"
        if nid not in self.nodes:
            self.nodes[nid] = KnowledgeNode(node_id=nid, entity=entity,
                entity_type=entity_type, properties=properties or {},
                created_at=datetime.now(timezone.utc).isoformat())
            self._save()
        return nid

    def add_edge(self, source_entity: str, target_entity: str,
                 relation: str = "related_to", weight: float = 1.0,
                 evidence: str = "") -> str:
        src_id = self.add_node(source_entity)
        tgt_id = self.add_node(target_entity)
        eid = f"e_{hashlib.md5(f'{src_id}-{relation}-{tgt_id}'.encode()).hexdigest()[:8]}"
        if eid not in self.edges:
            self.edges[eid] = KnowledgeEdge(edge_id=eid, source_id=src_id,
                target_id=tgt_id, relation=relation, weight=weight,
                evidence=evidence, created_at=datetime.now(timezone.utc).isoformat())
            self._save()
        else:
            self.edges[eid].weight = max(self.edges[eid].weight, weight); self._save()
        return eid
    ...
    def get_neighbors(self, entity: str) -> List[dict]:
        ...
        for edge in self.edges.values():
            if edge.source_id == nid:
                ...
                if t: neighbors.append({"entity": t.entity, "relation": edge.relation, "weight": edge.weight})
            elif edge.target_id == nid:
                s = self.nodes.get(edge.source_id)
                if s: neighbors.append({"entity": s.entity, "relation": f"inverse_{edge.relation}", "weight": edge.weight})
        return sorted(neighbors, key=lambda x: x["weight"], reverse=True)

    def find_path(self, from_entity: str, to_entity: str, max_depth: int = 5) -> List[str]:
        fid = self.add_node(from_entity); tid = self.add_node(to_entity)
        if fid == tid: return [from_entity]
        visited = {fid}; queue = [(fid, [from_entity])]
        depth = 0
        while queue and depth < max_depth:
            cid, path = queue.pop(0)
            for edge in self.edges.values():
                nid = None; nentity = None
                if edge.source_id == cid and edge.target_id not in visited:
                    nid = edge.target_id; t = self.nodes.get(nid); nentity = t.entity if t else "?"
                elif edge.target_id == cid and edge.source_id not in visited:
                    nid = edge.source_id; s = self.nodes.get(nid); nentity = s.entity if s else "?"
                if nid and nid not in visited:
                    p = path + [f"{edge.relation} -> {nentity}"]
                    if nid == tid: return p
                    visited.add(nid); queue.append((nid, p))
            depth += 1
        return []
...
class DualMemorySystem:
    ...
    def remember(self, content: str, tags: list = None,
                 importance: int = 5, source: str = "explicit") -> dict:
        auto_id = self.auto.store(content, source=source, tags=tags)
        cons_id = self.conscious.save(content, tags=tags, importance=importance, source=source)
    ...
    def recall(self, query: str, top_k: int = 5) -> dict:
        return {
            ...
            "knowledge_graph": self.kg.get_neighbors(query) if len(query.split()) <= 3 else [],
        }

    def consolidate(self) -> dict:
        promoted = self.conscious.consolidate_from_automatic(self.auto)
        return {"promoted": promoted, "auto_total": len(self.auto.entries),
                "conscious_total": len(self.conscious.entries)}

    def learn_relationship(self, e1: str, e2: str,
                           relation: str = "related_to", evidence: str = "") -> str:
        return self.kg.add_edge(e1, e2, relation, evidence=evidence)

    def get_stats(self) -> dict:
        return {"automatic": self.auto.get_stats(),
...
def get_dual_memory() -> DualMemorySystem:
    global _memory
    if _memory is None: _memory = DualMemorySystem()
    return _memory
...
if __name__ == "__main__":
    ...
    if cmd == "stats": print(json.dumps(mem.get_stats(), indent=2))
    elif cmd == "remember" and len(sys.argv) > 2:
        ids = mem.remember(sys.argv[2], importance=int(sys.argv[3]) if len(sys.argv) > 3 else 5)
        print(f"Stored: {ids}")
        ...
        print("Commands: stats, remember <text> [imp], recall <query>, consolidate, learn <e1> <e2> <rel>")