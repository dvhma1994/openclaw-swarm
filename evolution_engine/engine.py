...
BASE_DIR = Path(os.environ.get("OPENCLAW_DIRECTOR_DIR", Path.home() / ".openclaw" / "workspaces" / "director"))
...
class EvolutionPhase(str, Enum):
    IDLE = "idle"; DETECTING = "detecting"; MUTATING = "mutating"
    EVALUATING = "evaluating"; SELECTING = "selecting"
    PROMOTING = "promoting"; ROLLING_BACK = "rolling_back"

class MutationType(str, Enum):
    CODE_FIX = "code_fix"; CONFIG_TUNE = "config_tune"
    PROMPT_REFINE = "prompt_refine"; WORKFLOW_OPT = "workflow_opt"
    SAFETY_ENHANCE = "safety_enhance"; COST_REDUCE = "cost_reduce"

@dataclass
class FailurePattern:
    pattern_id: str; pattern_type: str; description: str
    occurrences: int = 1; last_seen: str = ""; first_seen: str = ""
    affected_components: list = field(default_factory=list)
    sample_error: str = ""; auto_fix_attempted: bool = False
    auto_fix_success: bool = False
    def to_dict(self): return asdict(self)

@dataclass
class Candidate:
    candidate_id: str; mutation_type: MutationType; target_component: str
    original_content: str = ""; mutated_content: str = ""
    description: str = ""; generation: int = 0
    fitness_score: float = 0.0; test_results: dict = field(default_factory=dict)
    created_at: str = ""; status: str = "pending"
    parent_id: Optional[str] = None; failure_pattern_id: Optional[str] = None
    def to_dict(self):
        return {**asdict(self), "mutation_type": self.mutation_type.value}
    @classmethod
    def from_dict(cls, data):
        data = dict(data); data["mutation_type"] = MutationType(data["mutation_type"])
        return cls(**data)

@dataclass
class EvolutionState:
    generation: int = 0; total_mutations: int = 0
    total_promotions: int = 0; total_rollbacks: int = 0
    current_phase: EvolutionPhase = EvolutionPhase.IDLE
    last_detection: str = ""; last_mutation: str = ""; last_promotion: str = ""
...
class PatternDetector:
    ...
    def _save(self):
        ...
        tmp_fd, tmp_path = tempfile.mkstemp(dir=str(self.failure_log.parent), suffix='.tmp')
        try:
    ...
    def record_failure(self, pattern_type: str, description: str,
                       component: str = "", error_text: str = "") -> str:
        error_sig = hashlib.md5(error_text[:200].encode()).hexdigest()[:8] if error_text else "noerr"
        ...
        if pattern_id in self.patterns:
            ...
            fp.occurrences += 1; fp.last_seen = now; fp.sample_error = error_text[:500]
            if component and component not in fp.affected_components:
                fp.affected_components.append(component)
        else:
            fp = FailurePattern(pattern_id=pattern_id, pattern_type=pattern_type,
                description=description, occurrences=1, first_seen=now, last_seen=now,
                affected_components=[component] if component else [], sample_error=error_text[:500])
            self.patterns[pattern_id] = fp
        self._save(); return pattern_id

    def get_recurring_failures(self, min_occurrences: int = 3) -> List[FailurePattern]:
        return sorted([p for p in self.patterns.values() if p.occurrences >= min_occurrences],
...
class MutationGenerator:
    ...
    def generate_code_fix(self, failure: FailurePattern, code_content: str, generation: int = 0) -> Candidate:
        cid = f"cfix_{failure.pattern_id[:20]}_g{generation}_{int(time.time())}"
        c = Candidate(candidate_id=cid, mutation_type=MutationType.CODE_FIX,
            target_component=failure.affected_components[0] if failure.affected_components else "",
            original_content=code_content,
            description=f"Auto-fix for recurring failure: {failure.description}",
            generation=generation, created_at=datetime.now(timezone.utc).isoformat(),
            failure_pattern_id=failure.pattern_id)
        self.candidates[cid] = c; self._save_candidate(c); return c

    def generate_config_tune(self, config_path: str, current_config: dict,
                              suggested_changes: dict, generation: int = 0) -> Candidate:
        cid = f"ctune_{hashlib.md5(config_path.encode()).hexdigest()[:8]}_g{generation}_{int(time.time())}"
        c = Candidate(candidate_id=cid, mutation_type=MutationType.CONFIG_TUNE,
    ...
    def generate_prompt_refine(self, prompt_name: str, current_prompt: str,
                                refined_prompt: str, generation: int = 0) -> Candidate:
        cid = f"prefine_{prompt_name[:15]}_g{generation}_{int(time.time())}"
        c = Candidate(candidate_id=cid, mutation_type=MutationType.PROMPT_REFINE,
            target_component=prompt_name, original_content=current_prompt,
            mutated_content=refined_prompt, description=f"Prompt refinement: {prompt_name}",
            generation=generation, created_at=datetime.now(timezone.utc).isoformat())
        self.candidates[cid] = c; self._save_candidate(c); return c
    ...
    def update_status(self, candidate_id: str, status: str, **kwargs):
        if candidate_id in self.candidates:
            c = self.candidates[candidate_id]; c.status = status
...
class FitnessEvaluator:
    ...
    def evaluate_code_fix(self, candidate: Candidate) -> dict:
        results = {"candidate_id": candidate.candidate_id,
                   "syntax_check": False, "import_check": False,
                   "fitness_score": 0.0, "errors": []}
        try:
            import ast; ast.parse(candidate.mutated_content)
            results["syntax_check"] = True
        except SyntaxError as e:
            results["errors"].append(f"Syntax error: {e}"); return results
        try:
            tree = ast.parse(candidate.mutated_content)
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        try: __import__(alias.name.split('.')[0])
                        except ImportError: results["errors"].append(f"Cannot import: {alias.name}")
                elif isinstance(node, ast.ImportFrom) and node.module:
                    try: __import__(node.module.split('.')[0])
                    except ImportError: results["errors"].append(f"Cannot import from: {node.module}")
            results["import_check"] = len(results["errors"]) == 0
        except Exception as e:
            results["errors"].append(f"Import check error: {e}")
        score = 0.0
        if results["syntax_check"]: score += 0.4
        if results["import_check"]: score += 0.3
        if not results["errors"]: score += 0.3
        results["fitness_score"] = min(1.0, score); return results

    def evaluate_config_tune(self, candidate: Candidate) -> dict:
        results = {"candidate_id": candidate.candidate_id,
                   "json_valid": False, "schema_check": False,
                   "fitness_score": 0.0, "errors": []}
        try:
            mutated = json.loads(candidate.mutated_content)
            results["json_valid"] = True
        except json.JSONDecodeError as e:
            results["errors"].append(f"Invalid JSON: {e}"); return results
        try:
        ...
        score = 0.0
        if results["json_valid"]: score += 0.5
        if results["schema_check"]: score += 0.5
        results["fitness_score"] = min(1.0, score); return results
    ...
    def evaluate(self, candidate: Candidate) -> dict:
        ...
        return {"candidate_id": candidate.candidate_id, "fitness_score": 0.5,
                "errors": [], "note": "Manual review required"}
...
class SelectionController:
    ...
    def select_for_promotion(self, evaluated: List[dict]) -> List[dict]:
        return [r for r in evaluated if r.get("fitness_score", 0) >= self.promote_threshold]
...
class RollbackManager:
    ...
    def create_snapshot(self, target_path: str) -> Optional[str]:
        ...
        meta = {"snapshot_id": sid, "target_path": target_path,
    ...
    def restore_snapshot(self, snapshot_id: str) -> bool:
        ...
        try:
            with open(sp, "r", encoding="utf-8") as f: meta = json.load(f)
    ...
    def cleanup_old_snapshots(self, max_age_days: int = 7):
        ...
        for f in self.snapshot_dir.glob("snap_*.json"):
            try:
                ...
                if datetime.fromisoformat(meta["created_at"]) < cutoff: f.unlink()
...
class EvolutionEngine:
    ...
    """
    def __init__(self, base_dir: Path = BASE_DIR):
        self.base_dir = base_dir; self.state = EvolutionState()
        self.detector = PatternDetector(); self.mutator = MutationGenerator()
        self.evaluator = FitnessEvaluator(base_dir); self.selector = SelectionController()
        self.rollback = RollbackManager(); self._load_state()
    ...
    def record_failure(self, pattern_type: str, description: str,
                       component: str = "", error_text: str = "") -> str:
        pid = self.detector.record_failure(pattern_type, description, component, error_text)
        self.state.last_detection = datetime.now(timezone.utc).isoformat()
        self._save_state(); return pid
    ...
    def propose_code_fix(self, failure: FailurePattern, code_content: str) -> Candidate:
        ...
        self._save_state(); return c

    def propose_config_tune(self, config_path: str, current: dict, suggested: dict) -> Candidate:
        c = self.mutator.generate_config_tune(config_path, current, suggested, self.state.generation)
        self.state.total_mutations += 1
        self.state.last_mutation = datetime.now(timezone.utc).isoformat()
        self._save_state(); return c

    def propose_prompt_refine(self, prompt_name: str, current: str, refined: str) -> Candidate:
        c = self.mutator.generate_prompt_refine(prompt_name, current, refined, self.state.generation)
        self.state.total_mutations += 1
        self.state.last_mutation = datetime.now(timezone.utc).isoformat()
        self._save_state(); return c

    def evaluate_candidate(self, candidate: Candidate) -> dict:
        self.state.current_phase = EvolutionPhase.EVALUATING; self._save_state()
        results = self.evaluator.evaluate(candidate)
        candidate.fitness_score = results.get("fitness_score", 0)
        candidate.test_results = results; candidate.status = "tested"
        self.mutator.update_status(candidate.candidate_id, "tested",
            fitness_score=results.get("fitness_score", 0), test_results=results)
        self.state.current_phase = EvolutionPhase.IDLE; self._save_state()
        return results

    def promote_candidate(self, candidate: Candidate) -> bool:
        if candidate.fitness_score < self.selector.promote_threshold: return False
        self.state.current_phase = EvolutionPhase.PROMOTING; self._save_state()
        snapshot_id = None
        ...
        try:
            ...
            if candidate.failure_pattern_id:
                self.detector.mark_fix_attempted(candidate.failure_pattern_id, success=True)
            self.state.total_promotions += 1
            self.state.last_promotion = datetime.now(timezone.utc).isoformat()
            self.state.generation += 1; self._save_state()
            return True
        except Exception:
            if snapshot_id: self.rollback.restore_snapshot(snapshot_id)
    ...
    def rollback_candidate(self, candidate: Candidate) -> bool:
        for sf in self.rollback.snapshot_dir.glob("snap_*.json"):
            try:
                with open(sf) as f: meta = json.load(f)
                if meta["target_path"] == candidate.target_component:
                    if self.rollback.restore_snapshot(meta["snapshot_id"]):
                        candidate.status = "rolled_back"
                        self.mutator.update_status(candidate.candidate_id, "rolled_back")
                        self.state.total_rollbacks += 1; self._save_state()
                        return True
            except Exception: pass
        return False

    def run_evolution_cycle(self) -> dict:
        report = {"cycle_start": datetime.now(timezone.utc).isoformat(),
                   "generation": self.state.generation,
                   "failures_detected": 0, "mutations_generated": 0,
                   "candidates_evaluated": 0, "promotions": 0, "errors": []}
        # Phase 1: Detect
        self.state.current_phase = EvolutionPhase.DETECTING; self._save_state()
        failures = self.get_recurring_failures()
        report["failures_detected"] = len(failures)
        # Phase 2: Mutate
        self.state.current_phase = EvolutionPhase.MUTATING; self._save_state()
        ...
        for c in pending:
            try:
                r = self.evaluate_candidate(c); evaluated.append(r)
                report["candidates_evaluated"] += 1
            except Exception as e: report["errors"].append(str(e))
        # Phase 4: Select & Promote
        self.state.current_phase = EvolutionPhase.SELECTING; self._save_state()
    ...
    def get_stats(self) -> dict:
        return {
            ...
            "promotion_rate": round(self.state.total_promotions / max(self.state.total_mutations, 1) * 100, 1),
...
if __name__ == "__main__":
    ...
    if cmd == "stats": print(json.dumps(engine.get_stats(), indent=2))
    elif cmd == "failures":
    ...
    elif cmd == "cycle": print(json.dumps(engine.run_evolution_cycle(), indent=2))
    elif cmd == "record" and len(sys.argv) > 3:
        pid = engine.record_failure(sys.argv[2], sys.argv[3],
            sys.argv[4] if len(sys.argv) > 4 else "",
            sys.argv[5] if len(sys.argv) > 5 else "")
        print(f"Recorded: {pid}")
    else: print("Commands: stats, failures, candidates, cycle, record <type> <desc>")
