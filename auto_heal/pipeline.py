    ...
    def _detect_tools(self) -> list:
        ...
        for tool in ["ruff", "flake8", "pylint", "python"]:
            try:
                ...
                logging.warning("Failed to detect lint tool %s", tool, exc_info=True)
        return tools
    ...
    def check(self, file_path: str) -> Dict[str, list]:
        ...
        if "ruff" in self.available_tools:
            try:
                ...
                logging.warning("Ruff check failed for %s", file_path, exc_info=True)
            ...
            try:
                ...
                logging.warning("Flake8 check failed for %s", file_path, exc_info=True)
        return errors
            ...
            if result.returncode != 0:
                ...
                for line in output.splitlines():
                    if "FAILED" in line or "ERROR" in line:
                        ...
                        errors["all"].append(
                            {"message": line.strip(), "type": "failure"}
                        )
                    if "ImportError" in line or "ModuleNotFoundError" in line:
                        errors["errors"].append({"message": line.strip()})
                        errors["all"].append(
                            {"message": line.strip(), "type": "import"}
                        )
...
class AutoFixer:
    ...
    def apply_fix(self, file_path: str, errors: list) -> dict:
        ...
        if not os.path.exists(file_path):
            report["remaining_errors"] = errors
            return report
...
class SelfHealPipeline:
    ...
    def heal_file(
        ...
        task_id = (
            f"heal_{hashlib.md5(file_path.encode()).hexdigest()[:8]}_{int(time.time())}"
        )
        start = time.time()
        result = HealResult(
            task_id=task_id,
        ...
        if result.status == HealStatus.FAILED:
            try:
                ...
                logging.warning(
                    "Failed to report healing failure to evolution engine",
                    exc_info=True,
                )
    ...
    def heal_directory(
        self, dir_path: str, pattern: str = "*.py", run_tests: bool = False
    ) -> dict:
            ...
            with open(HEAL_LOG, "r", encoding="utf-8") as f:
                for line in f:
                    try:
                        ...
                        logging.warning("Failed to parse heal log entry", exc_info=True)
        total = len(entries)
...
if __name__ == "__main__":
    ...
    if cmd == "stats":
        ...
        print("Commands: stats, heal <file> [--test], heal-dir <directory>")
