...
class HeadlessEvent:
    ...
    event_type: (
        str  # "start", "tool_call", "tool_result", "thinking", "error", "complete"
    )
...
class HeadlessRunner:
    ...
    def _emit(self, event_type: str, data: dict = None, duration_ms: int = 0):
        ...
        if self.output_format == OutputFormat.JSONL:
            ...
            if event_type == "tool_call":
                print(
                    f"  > {data.get('tool', '?')}: {str(data.get('input', ''))[:80]}",
                    flush=True,
                )
    ...
    def run_task(self, task: str, executor_fn=None) -> HeadlessResult:
        ...
        try:
            ...
            self._emit(
                "tool_call", {"tool": "cost_router", "input": {"task": task[:100]}}
            )
            tier = self._route_task(task)
            self._emit("tool_result", {"tool": "cost_router", "output": {"tier": tier}})
            ...
            self._emit(
                "complete", {"status": "completed", "output_length": len(str(output))}
            )
            ...
            self._emit(
                "error", {"message": str(e), "traceback": traceback.format_exc()[:500]}
            )
    ...
    def _route_task(self, task: str) -> str:
        ...
        try:
            ...
            valid, msg = registry.validate_execution(
                "cost_router", {"task_description": task}
            )
    ...
    def _default_executor(self, task: str, tier: str) -> str:
        ...
        self._emit(
            "tool_call",
            {"tool": "director", "input": {"task": task[:200], "tier": tier}},
        )
    ...
    def _finalize(self, result: HeadlessResult):
        ...
        result.total_tools = len(
            [e for e in self.events if e.event_type == "tool_call"]
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python runner.py <task> [--permission-mode yolo|autoEdit|plan] [--output-format text|json|jsonl]"
        )
    ...
    task = (
        sys.argv[1]
        if sys.argv[1] not in ("--permission-mode", "--output-format", "--input-file")
        else None
    )
    if task:
