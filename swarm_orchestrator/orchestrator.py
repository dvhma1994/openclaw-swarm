...
"""
import json, os, hashlib, time, tempfile, threading
...
class AgentRole(str, Enum):
    CODER = "coder"; REVIEWER = "reviewer"; PLANNER = "planner"
    DEBUGGER = "debugger"; RESEARCHER = "researcher"

class TaskPriority(str, Enum):
    CRITICAL = "critical"; HIGH = "high"; MEDIUM = "medium"; LOW = "low"

class VoteStrategy(str, Enum):
    MAJORITY = "majority"; BEST_SCORE = "best_score"
    CONSENSUS = "consensus"; WEIGHTED = "weighted"


@dataclass
class SwarmAgent:
    agent_id: str; role: AgentRole; model: str = "sonnet"
    reputation: float = 1.0; tasks_completed: int = 0
    tasks_correct: int = 0; last_active: str = ""
    def to_dict(self): return {**asdict(self), "role": self.role.value}
    @classmethod
    def from_dict(cls, data):
        data = dict(data); data["role"] = AgentRole(data["role"]); return cls(**data)
...
class SwarmTask:
    ...
    min_agents: int = 2; max_agents: int = 4; timeout_seconds: int = 300
    status: str = "pending"
    assigned_agents: list = field(default_factory=list)
    results: list = field(default_factory=list)
    winner: Optional[str] = None
    created_at: str = ""; completed_at: str = ""
    def to_dict(self): return {**asdict(self), "priority": self.priority.value}


@dataclass
class AgentResult:
    agent_id: str; task_id: str; content: str = ""
    score: float = 0.0; confidence: float = 0.5
    execution_time_ms: int = 0; error: str = ""; vote_weight: float = 1.0
    ...
    def post_insight(self, task_id: str, agent_id: str, insight: str):
        with self._lock:
            self.insights.setdefault(task_id, []).append({
                "agent_id": agent_id, "insight": insight,
                "timestamp": datetime.now(timezone.utc).isoformat()})
...
class ConsensusVoter:
    ...
    def vote(self, results: List[AgentResult]) -> Optional[AgentResult]:
        ...
        if not valid: return results[0]
        if self.strategy == VoteStrategy.MAJORITY: return self._majority_vote(valid)
        elif self.strategy == VoteStrategy.BEST_SCORE: return self._best_score_vote(valid)
        elif self.strategy == VoteStrategy.CONSENSUS: return self._consensus_vote(valid)
        elif self.strategy == VoteStrategy.WEIGHTED: return self._weighted_vote(valid)
    ...
    def _consensus_vote(self, results: List[AgentResult]) -> Optional[AgentResult]:
        ...
        if len(high) < len(results) and len(results) >= 2: return None
...
class SwarmOrchestrator:
    ...
    def register_agent(self, agent_id: str, role: AgentRole,
                       model: str = "sonnet") -> SwarmAgent:
        agent = SwarmAgent(agent_id=agent_id, role=role, model=model,
    ...
    def submit_task(self, description: str,
                    ...
                    min_agents: int = 2, max_agents: int = 4,
                    timeout_seconds: int = 300) -> SwarmTask:
        ...
        task = SwarmTask(task_id=task_id, description=description,
            priority=priority, required_roles=required_roles or [AgentRole.CODER.value],
            min_agents=min_agents, max_agents=max_agents,
            timeout_seconds=timeout_seconds,
            created_at=datetime.now(timezone.utc).isoformat())
        self.tasks[task_id] = task; return task
    ...
    def execute_agent(self, agent: SwarmAgent, task: SwarmTask,
                      executor_fn: Callable) -> AgentResult:
        start = time.time()
        try:
            other_obs = self.hub.get_observations(task.task_id, exclude_agent=agent.agent_id)
            context = {"observations": other_obs,
                       "insights": self.hub.get_insights(task.task_id)}
            result_content = executor_fn(agent, task, context)
            elapsed_ms = int((time.time() - start) * 1000)
            result = AgentResult(agent_id=agent.agent_id, task_id=task.task_id,
                content=str(result_content)[:5000], confidence=0.7,
                execution_time_ms=elapsed_ms, vote_weight=agent.reputation)
            self.hub.post_observation(task.task_id, agent.agent_id,
            ...
            elapsed_ms = int((time.time() - start) * 1000)
            return AgentResult(agent_id=agent.agent_id, task_id=task.task_id,
                error=str(e), execution_time_ms=elapsed_ms, vote_weight=agent.reputation)
    ...
    def run_task(self, task: SwarmTask, executor_fn: Callable) -> dict:
        ...
        with ThreadPoolExecutor(max_workers=len(agents)) as pool:
            futures = {pool.submit(self.execute_agent, a, task, executor_fn): a
                      for a in agents}
            for future in as_completed(futures, timeout=task.timeout_seconds):
                try:
                    ...
                    results.append(AgentResult(
                        agent_id=futures[future].agent_id, task_id=task.task_id,
                        error=str(e)))
        task.results = [r.to_dict() for r in results]
        winner = self.voter.vote(results)
        if winner:
            task.winner = winner.agent_id; task.status = "completed"
        ...
        with open(SWARM_LOG, "a", encoding="utf-8") as f:
            f.write(json.dumps({"task_id": task.task_id, "status": task.status,
                "agents": len(agents), "winner": task.winner}, default=str) + "\n")
        return {"task_id": task.task_id, "status": task.status, "winner": task.winner,
                "agents_used": [a.agent_id for a in agents],
                "winner_content": winner.content[:500] if winner else None}

    def run_parallel(self, description: str, executor_fn: Callable,
                     min_agents: int = 2, max_agents: int = 4) -> dict:
        task = self.submit_task(description, min_agents=min_agents, max_agents=max_agents)
    ...
    def get_stats(self) -> dict:
        return {
            ...
            "agents": {a.agent_id: {"role": a.role.value, "rep": a.reputation,
                       "tasks": a.tasks_completed, "correct": a.tasks_correct}
...
def get_swarm() -> SwarmOrchestrator:
    ...
    if _orchestrator is None: _orchestrator = SwarmOrchestrator()
...
if __name__ == "__main__":
    ...
    if cmd == "stats": print(json.dumps(swarm.get_stats(), indent=2))
    elif cmd == "agents":
        for a in swarm.agents.values():
            print(f"  {a.agent_id}: role={a.role.value} rep={a.reputation}")
    else: print("Commands: stats, agents")
